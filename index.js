// Prices changed to test price sorting function 
const state = {
  items: [
    {
      id: "001-beetroot",
...
function makeFilter() {
  ...
  div.classList.add('filter-section')
...
function createStoreFilter() {
  ...
  selectFilter.classList.add('filter-select')
...
function makeSorting() {
  ...
  div.classList.add('sort-section')
...
function createStoreSorting() {
  ...
  const sortPriceHighToLow = document.createElement('option');
  const sortPriceLowToHigh = document.createElement('option');
  ...
  sortPriceHighToLow.setAttribute('value', 'priceHighToLow'); 
  sortPriceHighToLow.innerText = 'Price: High to Low';
  sortPriceLowToHigh.setAttribute('value', 'priceLowToHigh'); 
  sortPriceLowToHigh.innerText = 'Price: Low to High';

  selectSorting.append(defaultSorting)
  selectSorting.append(sortAlphabetically)
  selectSorting.append(sortPriceHighToLow)
  selectSorting.append(sortPriceLowToHigh)

  selectSorting.addEventListener('change', () => sortStoreItems(selectSorting.value))

  selectSorting.classList.add('sort-select')

  return selectSorting
}

function sortStoreItems(value) {
  if (value === 'alphabetically') { // Sort A-Z
    state.items.sort(function(a, b) {
      return a.name.localeCompare(b.name);
    });
    addItemToStore(state.items);
  } else if (value === 'priceHighToLow') { // Sorting high to low
    state.items.sort(function(a, b) {
      return b.price - a.price;
    });
    addItemToStore(state.items);
  } else if (value === 'priceLowToHigh') { // Sorting low to high
    state.items.sort(function(a, b) {
...
// Create form (I HAVE ADDED A STRAWBERRY SVG TO ASSET FILE FOR TESTING )
function addNewItem() {
...
function createForm() {
  ...
  const itemImagePreview = document.createElement('img')
  ...
  itemImageLabel.innerText = 'Item image (SVG)'
  itemImageLabel.classList.add('label')
  ...
  itemImageInput.setAttribute('type', 'file')
  itemImageInput.setAttribute('accept', 'image/svg+xml') 
  ...
  itemImagePreview.style.maxWidth = '200px'
  itemImagePreview.style.maxHeight = '200px'
  ...
  form.append(itemImageLabel)
  form.append(itemImageInput)
  form.append(itemImagePreview)
  ...
  form.addEventListener('submit', (event) => {
    ...
    formData.append('type', itemTypeSelect.value)
